...
class Article_Controller extends Bl_Controller
{
  ...
  {
  	$pageRows = $this->_displayInfo['articleListNum'] ? $this->_displayInfo['articleListNum'] : 20;
  	$filter = array();
  	$matches = array();
...
/*    if($path == 'all.html'){
    ...
    $articleList = $this->_articleInstance->getArticleList($filter, $page, $pageRows);
    $articlecount = $this->_articleInstance->getArticleCount($filter);
    $urlPage = 'articletype/' . $path_alias . '_%d_' . $pageRows . '.html';
    ...
    $this->view->render('articlelist.phtml', array(
     ...
     'pagination' => callFunction('pagination', $urlPage, $articlecount, $pageRows, $page, $firstPath),
    ));
