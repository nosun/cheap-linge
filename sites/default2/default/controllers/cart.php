...
class Cart_Controller extends Bl_Controller
{
  ...
  {
  	if ($this->isPost()) {
  		log::save('debug', 'in post', $_POST);
  		$post = $_POST;
	  	if (isset($post['checkout'])) {
	  		log::save('debug', 'gotourl', 'cart/checkout/'.$post['pid'].'/0/'.$post['qty']);
  ...
  public function ajaxGetFeesAction($pids, $fee_names, $cid, $pid){
      ...
      if(($payment == 'not_set')){
        ...
        if (isset($paymentInfo->bankfee) && $paymentInfo->bankfee) {
          if (strpos($paymentInfo->bankfee, '%')) {
            $paymentInfo->bankfee = floatval($paymentInfo->bankfee);
            $bank_fee = $paymentInfo->bankfee * ($cartInfo->goods_amount +  $shipping_fee) / 100;
            $bank_fee = ceil_dec($bank_fee, 2);
          } else {
            $paymentInfo->bankfee = floatval($paymentInfo->bankfee);
            $bank_fee = $paymentInfo->bankfee;
          }
        }
      }else{
        $bank_fee = -1;
