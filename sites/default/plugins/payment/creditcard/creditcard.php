...
class CreditCard extends Payment_Abstract implements Payment_Interface, Payment_Server_Interface
{
  ...
  public function getSubmitForm($info)
  {
    ...
    $returnUrl = url('payment/callback/creditcard');
    ...
    $html = array(
    		'<form action="' . $paymentInfo->submitUrl . '" method="post" target="_blank" >',
    		...
    		'<input type="submit" class="pay_button" value="Pay Now"/>',
    		'</form>',);
  ...
  public function callback($orderNumber=null)
  {
	...
	if ($payment_status == 1) {
		...
		$result['message'] = 'Sorry, your payment could not be completed with the chosen credit card , For the reason of:' . $post['orderInfo'] . '. Please try again, use another card or choose another payment method.';
	}
	return $result;
  }
