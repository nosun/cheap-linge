...
class Order_Controller extends Bl_Controller
{
  ...
  private function getPaymentList($orderInfo) {
  	$paymentInstance = Payment_Model::getInstance();
  	$paymentList = $paymentInstance->getPaymentsList();
  	unset($paymentList['wiretransfer']);
  	unset($paymentList['western']);
  ...
  public function infoAction($oid)
  {
    ...
    if($orderInfo->uid != $user->uid){
	    ...
	    $this->view->assign('tmark', 'order');
	    $this->view->assign('templatefile', 'u_orderinfo.phtml');
	    $this->view->render('personalcenter.phtml', array(
	      'orderInfo' => null,
  ...
  /*
  public function outboundAction($orderNumber) {
  	$order = $this->_orderInstance->getOrederInfoByNumber($orderNumber);
  	$order->items = $this->_orderInstance->getOrderItems($order->oid);
  	
  	Shipping_Model::getInstance()->createAmazonShippingOrder($order);
  }
	*/
}