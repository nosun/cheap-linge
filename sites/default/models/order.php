...
class Order_Model extends Bl_Model
{
   ...
   * @param int $pageRows
   */
  public function getOrdersList($post = array(), $page, $pageRows)
  {
    global $db;
    $filter = array(
   ...
   * @param int $order_id
   */
  public function updateOrder($oid, $post, $recompute=true)
  {
    global $db;
      $post['fee_amount'] = 0;
    if(isset($post['fees'])){
    ...
    if(isset($post['status_payment'])){
    	$set['status_payment'] = $post['status_payment'];
    }
    if(isset($post['status_shipping'])){
    	$set['status_shipping'] = $post['status_shipping'];
    }
    if(isset($post['shipping_method'])){
    	$set['shipping_method'] = $post['shipping_method'];
    }
    if(isset($post['delivery_first_name'])){
    	$set['delivery_first_name'] = $post['delivery_first_name'];
    }
    if(isset($post['delivery_last_name'])){
    	$set['delivery_last_name'] = $post['delivery_last_name'];
    }
    if(isset($post['delivery_mobile'])){
    	$set['delivery_mobile'] = $post['delivery_mobile'];
    }
    if(isset($post['delivery_phone'])){
    	$set['delivery_phone'] = $post['delivery_phone'];
    }
    if(isset($post['delivery_email'])){
    	$set['delivery_email'] = $post['delivery_email'];
    }
    if(isset($post['delivery_time'])){
    	$set['delivery_time'] = $post['delivery_time'];
    }
    if(isset($post['delivery_postcode'])){
    	$set['delivery_postcode'] = $post['delivery_postcode'];
    }
    if(isset($post['delivery_city'])){
    	$set['delivery_city'] = $post['delivery_city'];
    }
    if(isset($post['delivery_address'])){
    	$set['delivery_address'] = $post['delivery_address'];
    }
    if(isset($post['pay_amount'])){
    	$set['pay_amount'] = $post['pay_amount'];
    }
    if(isset($post['fee_amount'])){
    	$set['fee_amount'] = $post['fee_amount'];
    }
    if(isset($post['data']) && is_array($post['data'])){
    	$set['data'] = serialize($post['data']);
    }
  ...
  public function updateOrderDataField($oid, $data){
  	global $db;
  	$db->update('orders', array('data'=>serialize($data), 'updated' => TIMESTAMP), array('oid' => $oid));
  	cache::remove('order-' . $oid);
  	return $db->affected();
  }
 ...
 /**
  ...
  public function updateOrderItemShippingNo($oiids, $shipping_no){
  	global $db;
  	$db->query('update orders_items set shipping_no = "'. $shipping_no . '" where oiid in ('. implode(',', $oiids).')');
  }
  ...
  private function getOrderStatistics($oid, $isfee = 1, $ifItem = 1)
  {
    ...
    if (isset($set['total_amount'])) {
    	$set['total_amount'] = $set['total_amount'] > 200 ? $set['total_amount'] * 0.6 : $set['total_amount'];
    } else {
    	$set['total_amount'] = $orderInfo->fee_amount;
    }*/
    if (!isset($set['total_amount'])) {
    	$set['total_amount'] = $orderInfo->fee_amount;
    }
  ...
  public function dealOrderStatus($result)
  {
  	...
  	if ($result['verified'] == 1) {
  		...
  		if ($this->updateStatusOrder($orderInfo->oid, array('status_payment' => 1, 'payment_method' => $result['payment_method']))) {
  			return true;
  		}
  	} else {
  		return -1;
