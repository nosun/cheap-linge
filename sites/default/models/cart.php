...
class Cart_Model extends Bl_Model
{
  ...
  public function getCartProductList($cart_item_id = null, $page = 1, $pageRows = null, $ifStatShipping = true)
  {
      ...
      foreach ($products as $k => &$product) {
        ...
        if ($productinfo) {
          ...
          if(isset($productinfo->price)){
          	...
          	$product->price = $productinfo->sell_price;
          }
          //added by pzzhang.
    ...
    //}
      ...
      // 对于超过200的客户打六折
      /*
      if (isset($list[$sid]) && isset($list[$sid]->goods_amount) && $list[$sid]->goods_amount > 200) {
      	$list[$sid]->goods_amount *= 0.6;
      }
      */
    return $list[$sid];
  }
  ...
  public function insertProductToCart($post)
  {
    ...
    if ($set['qty']) {
        if (empty($set['data'])) {
            log::save('cart', 'cart data is empty', debug_backtrace());
        }
