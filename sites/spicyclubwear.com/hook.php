...
//  $instance->view->assign('helpList', $helpList);
     ...
     //文章列表
  $news_list= $frontInstance->getArticleList(array('type_id'=>'news','status'=>1),1,7);
  ...
  foreach($articleTypeList as $index=>$type){
  	...
  	$articleList = $frontInstance->getArticleList(array('type_id'=>$type->type_id,'status'=>1),1,4);
...
function hook_default_index(Default_Controller $instance)
{
	...
	$hdp = $siteInsance->getcarouselphotoList(1,6);
  ...
  $instance->view->assign('hotProductList', $hotProductList);
   ...
   //获取新品列表
   $newProductListBySpecial = $frontInstance->getProductsListBySpecial(array('termname' =>'New Arrivals'), 1, 40);
   $newProductList = $frontInstance->getProductsList(array('termname' =>'New Arrivals'), 1, 40);
   $remains = 40 - count($newProductListBySpecial);
  ...
  //页脚推荐产品列表
  $recommendProductList = $frontInstance->getProductsListBySpecial(array('termname' =>'Recommended'), 1, 20);
  ...
  $disProductList = $frontInstance->getProductsListBySpecial(array('termname' =>'Discounted'), 1, 20);
...
function hook_getrandom($op, $instance, $filter)
{
  if ($op == 'browse') {
    ...
    $filter['level'] = 'self';
    $similarproduct = widgetCallFunction('randominfo', 'getinfo', 'products', $filter, 20);
  } elseif ($op == 'product') {
    $filter['directory_tid'] = $filter['tid'];
    $filter['level'] = 'self';
    $similarproduct = widgetCallFunction('randominfo', 'getinfo', 'products', $filter, 20);
    ...
    $similarproduct = widgetCallFunction('randominfo', 'getinfo', 'products_2', null, 20);
  }
